  ...
  getSearchResults: async (queryString) => {
    if(queryString.includes('&q=')){
      ...
      if(response.data.response.numFound === 0) {
        response = await Api.get(`/spell?facet.field=tipoAsunto_facet&facet.field=secretario_facet&facet.field=ponente_facet&facet=on${queryString}&fl=*&hl.fl=data_file&hl.snippets=10&hl=on&wt=json`)
      }
      return response;
    }
